...
unsigned char AHT10_Get_Humidity_Temperature(float *humidity, float *temperature)
{
	esp_err_t err = ESP_OK;
	...
	{
		hum = ((aht10_data[1]<<12) | (aht10_data[2]<<4) | (aht10_data[3]>>4));
		temp = (((aht10_data[3]&0x0F)<<16) | (aht10_data[4]<<8) | (aht10_data[5]));

		*humidity = (hum *100.0 / 1024/ 1024 + 0.5);
